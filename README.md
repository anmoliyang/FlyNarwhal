...
**支持使用 GPU 硬解，但是因为框架渲染机制，会将硬解后的视频转为 RGBA 色彩模式，此时高动态范围视频会产生色彩映射错误的问题，造成画面色彩显示异常。所以对于 HDR、HLG 以及 Dolby Vision 格式的视频，此客户端将强制 NAS 映射为 SDR 输出，对于硬件性能不足的 NAS 可能无法流畅播放。**
